...
export class MyDatePicker implements OnChanges, ControlValueAccessor, OnInit {
    ...
    range() {
      if( this.rangeDate ){
        ...
        if( ! this.startingPoint.day ) {
          console.log('here');
          return this.opts.componentDisabled = this.disabled = true;
        }
        this.opts.componentDisabled = this.disabled = false;
    ...
    ngOnChanges(changes: any) {
      if( this.rangeDate && changes.startingPoint ){
        console.log('changes starting', this.startingPoint)
        this.range();
      }
        if( this.rangeDate && changes.startingPoint ) {

          if( this.includeSelectedDate ){
            if( this.startingPoint.day > 1) this.startingPoint.day -=1;
            else{
