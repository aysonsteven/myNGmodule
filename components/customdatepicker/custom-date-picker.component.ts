...
export class MyDatePicker implements OnChanges, ControlValueAccessor, OnInit {
  ...
  @Input() includeSelectedDate: boolean = true;
  @Input() rangeDate: boolean = false;
  @Input() minYear: number = 1100;
  @Input() maxYear: number = 9100;
  ...
  @Input() toStringDate: boolean = false;
  @Input() startingPoint: IMyDate = <IMyDate>{ year:0, month:0, day:0 };
  @Input() endingPoint: IMyDate = <IMyDate>{ year:0, month:0, day:0 };
  @Output() objectBinder = new EventEmitter();
  ...
  constructor(
    ...
    renderer.listenGlobal("document", "click", (event: any) => {
      if ( this.closeOnBlur && this.showSelector && event.target && this.elem.nativeElement !== event.target && !this.elem.nativeElement.contains(event.target)) {
  ...
  getDateVal(){
    if( this.inputDate ){
      let newDate = new Date( 1504764067000 );
      let dateValue: IMyDate = <IMyDate>{};
      dateValue.year = newDate.getFullYear();
      dateValue.month = newDate.getMonth() +1;
      dateValue.day = newDate.getDate();
      console.log( 'dateval', dateValue );
      this.selectDate( dateValue, CalToggle.CloseByDateSel );

    }
  }
  ngOnInit(){
    this
    // this.startingPoint.year = new Date( this.inputDate ).getFullYear();
    // this.startingPoint.month = new Date( this.inputDate ).getDate() + 1;

    this.opts.dateFormat = this.dateFormat;
    this.range();
    this.opts.height = this.elementHeight;
  }
  ...
  range(  ) {
    if( this.rangeDate ){
      if( !this.inputDate ) this.clearDate();
      if( !this.startingPoint.day ){
        this.selectedDate = <IMyDate>{year: 0, month: 0, day: 0};
  ...
  setOptions(): void {
    ...
    if (this.opts.minYear < this.minYear) {
      this.opts.minYear = this.minYear;
    }
    if (this.opts.maxYear > this.maxYear) {
      this.opts.maxYear = this.maxYear;
    }
  ...
  onSelectYearClicked(event: any): void {
    ...
    this.selectMonth = false;
    this.cdr.detectChanges();
    if (this.selectYear) this.generateYears(this.visibleMonth.year);
  }
  ...
  ngOnChanges(changes: any) {
    ...
    if( this.rangeDate && changes.startingPoint ){
      if( this.startingPoint.day>0 ) console.log('changes starting', this.startingPoint)
      this.clearDate();
    ...
    if( this.rangeDate && changes.startingPoint ) {
      this.startingPoint = JSON.parse( JSON.stringify( this.startingPoint ));
      if( this.includeSelectedDate ){
        if( this.startingPoint.day > 1 ) {
          this.startingPoint.day -=1;
          console.log( 'month', this.startingPoint )
        }
        else{

          this.startingPoint.month -=1;
          this.startingPoint.day = this.daysInMonth( this.startingPoint.month, this.startingPoint.year );
        }
      }
      this.opts.disableUntil = this.startingPoint;
  ...
  updateDateValue(date: IMyDate, clear: boolean): void {
    ...
    let dateValue = this.formatDate(date);
    let dateValueString = new Date(this.formatDate( date ));


    if( dateValueString  !instanceof Date && isNaN(dateValueString.valueOf()) ) dateValueString = null;
    if( ! this.toStringDate && dateValue =='0/00/00' ) dateValue = null


    let date2 =  {value: this.formatDate(date), dateFormat: this.opts.dateFormat, valid: !clear}
    this.selectionDayTxt = clear ? "" : this.formatDate(date);
    if( ! this.toStringDate) this.inputFieldChanged.emit({value: dateValue, dateFormat: this.opts.dateFormat, valid: !clear});
    else this.inputFieldChanged.emit({value: dateValueString, dateFormat: this.opts.dateFormat, valid: !clear});
    this.objectBinder.emit( date );
    this.invalidDate = false;
    let dateFormatted = new Date( this.formatDate(date))
    this.testreturn = dateValue;
    console.log('date', {value: dateValue, dateFormat: 'mm-dd-yyyy', valid: !clear})
  }
  ...
  daysInMonth(m: number, y: number): number {
    ...
    console.log('days inmonth', new Date(y, m, 0).getDate() );
